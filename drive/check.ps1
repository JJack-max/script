...
$ConfigFile = Join-Path $PSScriptRoot 'check_his.json'

function Select-FolderDialog {
    $dialog = New-Object System.Windows.Forms.FolderBrowserDialog
    $dialog.ShowNewFolderButton = $true
    $dialog.Description = '请选择要校验的本地文件夹'
...
# 选择目录
$localInputText = Read-Host "输入要校验的本地目录编号（可用空格分隔多选，直接回车为全部，最后一个选项为新目录）："
$SelectedLocalPaths = @()
$needAddNew = $false
...
$inputText = Read-Host "输入要校验的远程编号（可用空格分隔多选，直接回车为全部）："
...
foreach ($localPath in $SelectedLocalPaths) {
    ...
    foreach ($remote in $Selected) {
        Write-Host "--------------------------------------------"
        Write-Host "Checking $localPath <-> $remote/$dirName ..."
        $result = & rclone check $localPath "${remote}:$dirName" 2>&1
        Write-Host $result
        if ($LASTEXITCODE -eq 0) {
            Write-Host "✔️ 校验通过：$localPath <-> $remote/$dirName"
        } else {
            Write-Host "❌ 差异或错误：$localPath <-> $remote/$dirName"
        }
    }
}

Write-Host "All check tasks are complete."
