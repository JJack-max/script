...
get_main_ip_address() {
  if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    ip_address=$(ifconfig "$interface" | grep 'inet6 ' | grep -v 'fe80' | awk '{print $2}' | head -n 1)
  else
    interface=$(ip route | grep default | awk '{print $5}' | head -n 1)
    ip_address=$(ip addr show "$interface" | grep 'inet6 ' | grep -v 'fe80' | awk '{print $2}' | cut -d'/' -f1 | head -n 1)
  fi
...
get_turn_external_ip() {
  ...
  IP=$(curl -s -6 https://jsonip.com | jq -r '.ip')
  if [[ "x-$IP" != "x-" ]]; then
...
http://${NETBIRD_DOMAIN} {
    ...
    redir https://${NETBIRD_DOMAIN}{uri} permanent
}
EOF
}
